...
const NavResp = () => {
  ...
  return (
  <>
  <div className={status?"duration-1000 transition nav-animate-open ease-in-out lg:hidden md:hidden sm:flex flex flex-row flex-wrap bg-white w-full h-screen shadow-md shadow-gray-400 fixed  z-50":"transition ease-in-out duration-1000 lg:hidden md:hidden sm:flex flex flex-row flex-wrap bg-white w-full h-24 shadow-md shadow-gray-400 fixed  z-50 nav-animate-open"}>
    <div className="flex  justify-start w-1/3 m-auto h-15 p-5 content-start h-24">
        <Image src="https://i.ibb.co/RGF1mW7/navlogo.png"  width={80} height={80} className='w-20   pl-6 ml-5' alt='logo'loading='lazy'/> 
    </div>
